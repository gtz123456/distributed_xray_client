...
fn launch_xray(uuid: String, pubkey: String, server: String) -> String {
  ...
  let mut default_config = {  
    let file_content = fs::read_to_string("./xray.json").expect("LogRocket: error reading file");
  ...
  // for windows, xray is a .exe file so we need to add the extension
  let xray_path = if cfg!(target_os = "windows") {
    "./bin/xray.exe"
  } else {
    "./bin/xray"
  };
  let xray = std::process::Command::new(xray_path).spawn().expect("LogRocket: error launching xray");
